  ...
  return (
    <main>
      {introState ? (
        ...
        <div className="questions">
          <img className="blob1-start" src={Blob1} />
          <img className="blob2-start" src={Blob2} />
          {quizQuestions}
          {answeredQuestions === 5 ? (
            <div class="score">
              <h2 className="finalScore">You have Scored {correctAnswers}/5</h2>
              <button className="qbutton" onClick={newQuestions}>
                Play Again
              </button>
            </div>
          ) : checked === false ? (
            <button className="qbutton" onClick={checkAnswers}>
              Check Answers
            </button>
          ) : (
            <button className="qbutton" onClick={newQuestions}>
